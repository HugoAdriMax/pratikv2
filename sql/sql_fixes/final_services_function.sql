-- Version finale avec référence non ambiguë

-- Drop la fonction
DROP FUNCTION IF EXISTS public.get_prestataire_services(UUID);

-- Créer la fonction avec un paramètre clairement nommé
CREATE FUNCTION public.get_prestataire_services(p_prestataire_id UUID)
RETURNS TABLE(
    id UUID,
    name TEXT,
    category TEXT,
