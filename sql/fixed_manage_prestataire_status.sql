-- Correction de la fonction RPC pour résoudre l'ambiguïté avec "notes"
...
BEGIN
  ...
  IF current_activation_id IS NOT NULL THEN
    ...
    SET 
      status = new_status,
      admin_id = admin_id,
      notes = COALESCE(notes_param, prestataire_activations.notes),
      updated_at = now()
  ...
  SET 
    ...
    is_verified = CASE 
      WHEN new_status = 'approved' THEN TRUE 
      ELSE users.is_verified -- ne changez pas is_verified si le statut est rejeté
    END
