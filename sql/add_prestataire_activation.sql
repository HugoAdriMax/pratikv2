-- Ajouter la colonne is_active à la table users si elle n'existe pas déjà
...
DROP POLICY IF EXISTS "Users can update their is_active status" ON public.users;
CREATE POLICY "Users can update their is_active status"
ON public.users
FOR UPDATE
TO authenticated
USING (id = auth.uid())
WITH CHECK (id = auth.uid() AND (is_active = OLD.is_active));
